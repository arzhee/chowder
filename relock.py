...
def relock(address, vault, amount):
    global count
    ...
    try:
        ...
        print('[INFO]', 'Adding PEARLs to Note #' + str(vault), 'in', nonce, 'nonce...')
        ...
        tx = lake.functions.extendLock(address, vault, amount).buildTransaction(tx)
        ...
        print('[FAIL]', 'An error occured on locking PEARLs **')

        return relock(address, vault, amount)
...
for name, note in notes.items():
    ...
    for index in range(items):
        ...
        locked = get_locked(note, vault)
        ether = w3.fromWei(locked, 'ether')
