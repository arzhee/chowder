...
def get_pearl(address, vault, type):
    value = None

    try:
        if type == 0:
            value = lake.functions.balanceOf(address, vault).call()

            return value

        value = lake.functions.reward(address, vault).call()
    except Exception as e:
        return get_pearl(address, vault, type)
...
for name, note in notes.items():
    ...
    for index in range(items):
        ...
        locked = get_pearl(address, vault, 0)
        reward = get_pearl(address, vault, 1)

        print('[INFO]', detail, '#' + str(vault))

        total = total + (locked + reward)

        locked = w3.fromWei(locked, 'ether')
        reward = w3.fromWei(reward, 'ether')

        print('[INFO] Locked:', locked, 'PEARL')
        print('[INFO] Reward:', reward, 'PEARL')

amount = w3.fromWei(total, 'ether')

print('----------------------------------------')
print('[INFO] Total:', amount, 'PEARL')

total = pearl.functions.pearlTosCLAM(total).call()
total = w3.fromWei(total, 'gwei')

print('[INFO] sCLAM:', total, 'sCLAM')